...
exports.fetchPlanning = function(req, res) {
    var basequery = "SELECT s.id AS idschedule, s.dayid AS dayid, s.timeframe AS timeframe, s.setting AS setting, s.game , s.player AS player, s.role, g.title, g.time FROM schedule s LEFT JOIN game g ON s.game=g.id WHERE (s.dayid >= $1) AND (s.dayid <= $2)";
