...
exports.fetchAllUsersRaw = function(successcallback, errorcallback) {
    playerData.orderBy('name', persist.Ascending).all(connection, function(err, users) {
        if (err) {
            errorcallback(err);
        }
...
exports.fetchAllUsers = function(req, res) {
    exports.fetchAllUsersRaw(function(result) {
        res.send(result);
    }, function(err) {
        res.send(500, "Error: " + err);
    });
...
exports.cancelEvent = function(req, res) {
    schedule.where({ timeframe : req.params.timeframe, dayid : req.params.dayid, setting : req.params.setting}).deleteAll(connection, function(err) {
        if (err) {
            res.send(500, "Error: " + err);
        }
        else {
            comment.where({ timeframe : req.params.timeframe, dayid : req.params.dayid, setting : req.params.setting}).deleteAll(connection, function(err) {
                if (err) {
                    res.send(500, "Error: " + err);
                }
                else {
                    var logdata = createBaseLogData(req, req.params);
                    storelog(logdata);
                    res.send("OK");
                }
            });
...
function createEventSchedule(dayid, timeframe, setting, errorcallback, successcallback) {

    var newschedule = new schedule({ dayid : dayid, timeframe : timeframe, role : "GM", setting : setting, player : "__EVENTS__"});

    newschedule.save(connection, function(err) {
        if (err)
            errorcallback(err);
        else {
            var rawgame = {
                masterschedule : newschedule.id,
                time : "A DEFINIR",
                title : "A DEFINIR"
            };
            ...
            newgame.save(connection, function(err) {
                if (err)
                    ...
                    newschedule.game = newgame.id;
                    newschedule.save(connection, function(err) {
                        if (err)
                            errorcallback(err);
                        else
                            successcallback(newgame.id)
                    });
                }
            });
        }
...
function handleEventSchedule(req, res, newschedule, errorcallback, successcallback) {
    schedule.where({ dayid : newschedule.dayid, timeframe : newschedule.timeframe, setting : newschedule.setting}).first(connection, function(err, result) {
        if (err) {
            errorcallback(err);
            return;
        }
        if (result == null) {
            if (!req.admin) {
                errorcallback("L'ajout d'évènements est réservé aux administrateurs");
                return;
            }
            createEventSchedule(newschedule.dayid, newschedule.timeframe, newschedule.setting, function (err) {
                errorcallback(err);
            }, function (gameid) {
        ...
        successcallback(result.game);
    });

}

function storeSchedule(req, res, newschedule) {
    newschedule.save(connection, function(err) {
        if (err)
            res.send(500, "Error: " + err);
        else {
            var logdata = createBaseLogData(req, newschedule);
            logdata.data = {
                role : newschedule.role
            };
            storelog(logdata);
            genericSendJson(res, newschedule);
        }
     });
}
...
exports.createSchedule = function(req, res) {
    ...
    setting.getById(connection, newschedule.setting, function(err, setting) {
        if (err) {
            res.send(500, "Error: " + err);
        }
        else {
            if (setting.mode == 3) {
                ...
                handleEventSchedule(req, res, newschedule, function(err) {
                    res.send(500, "Error: " + err);
                    return;
                }, function(gameid) {
//                    newschedule.game = gameid;
                    storeSchedule(req, res, newschedule);
                });
            }
            else {
                storeSchedule(req, res, newschedule);
...
exports.fastlogin = function(username, password, successcallback, errorcallback) {
    playerData.where({ name : username}).first(connection, function(err, result) {
        if (err) {
            errorcallback(err);
            return;
        }
        if (result == null) {
            errorcallback(username + '? Connais pas!');
            return;
        }

        var passSum = security.hashPassword(password);
        if (result.password != passSum) {
            errorcallback('Enlève tes moufles et retape ton mot de passe');
            return;
        }

        if (result.status > 0) {
            errorcallback('Compte désactivé');
            return;
        }
...
exports.updateEvent = function(req, res) {
    fetchCompleteSchedulesByGame(req.params.id, function(err, schedules) {
        if (err)
            res.send(500, "Error: " + err);
        else {
            game.update(connection, req.params.id, req.body.game, function (err) {
                if (err)
                    res.send(500, "Error: " + err);
                else {
                    res.send(req.body.game);
                    var logschedule = {
                        dayid : schedules[0].dayid,
                        timeframe : schedules[0].timeframe,
                        setting : schedules[0].setting
                    };
                    var logdata = createBaseLogData(req, req.body.schedule);
                    logdata.data = req.body.game;
                    storelog(logdata);
                }
            });
        }
